...
const runInviteReminder = async (label = 'Scheduled') => {
    try {
      const now = new Date();
      const reminderThreshold = new Date(now.getTime() + 24 * 60 * 60 * 1000); // 24 hours ahead
  
      const invites = await Invite.find({
        expiresAt: { $gte: tomorrow, $lte: endOfTomorrow },
        reminderSent: false,
        revoked: false,
      ...
      console.log(`[${new Date().toISOString()}] ✅ ${label} - Sent ${invites.length} invite reminders.`);
    } catch (err) {
      console.error(`[${new Date().toISOString()}] ❌ ${label} - Failed to send invite reminders:`, err.message);
    }
  };
  
  // Run once on server start
  runInviteReminder('Startup');
  
  // Schedule to run every hour
  cron.schedule('0 * * * *', () => runInviteReminder('Scheduled'));
  
  module.exports = runInviteReminder;